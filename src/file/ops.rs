...
pub fn save_page<P>(page: &Page, path: &P) -> Result<(), FileError>
where
    P: AsRef<Path>,
{
    let path = path.as_ref();
    fs::write(&path, &page.html).map_err(|source| PathError::new(&path, source))?;
    info!(path = %path.display(), length = page.html.len(), "save HTML file");
    Ok(())
}
